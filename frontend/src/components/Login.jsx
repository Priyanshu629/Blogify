    ...
    axios
      ...
      .catch((error) => {
        if (error.response && error.response.status === 422) {
          alert("Invalid username or password or account doesnot");
        }
      });
  };
  return (
    <div className="d-flex  w-100 vh-100 align-items-center justify-content-center">
      <div className="animate" id="animatediv"></div>
      <div
        className="w-45  p-3 rounded-3 "
        style={{ backgroundColor: "aqua" }}
        id="logindiv"
      >
        <h1 className="text-center">Welcome to Blogify</h1>
        <p style={{ fontStyle: "italic" }}>
          Make an account and start writing blogs and share your knowledge
        </p>
        ...
        <Link to={"/register"} className="btn btn-success mt-2 w-100">
          Register
        </Link>
      </div>
    </div>
